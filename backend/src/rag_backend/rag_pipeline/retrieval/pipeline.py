...
async def run_retrieval(
    ...
    payload (see step10_response). Every step logs before and after it runs to
    the console; the full system prompt is logged right before the LLM call.

    The entire request — user message, system prompt, retrieved chunks, final
    answer, and per-step timings (or the error, if one occurred) — is also
    written to a single JSON file under pipeline-logs/retrieval/ once the request
    finishes, so one exchange can be inspected without grepping interleaved logs.
    """
    ...
    try:
        ...
        _log_step_end("4_similarity_search", started, timings_ms)

        started = _log_step_start("5_metadata_filter")
        filtered_chunks = apply_metadata_filter(scored_chunks, embedded_query)
        _log_step_end("5_metadata_filter", started, timings_ms)

        started = _log_step_start("6_reranking")
        reranked_chunks = rerank(filtered_chunks)
        _log_step_end("6_reranking", started, timings_ms)

        started = _log_step_start("7_combine_context")
        context = await combine_context(reranked_chunks)
        record["citations"] = [citation.model_dump() for citation in context.citations]
        _log_step_end("7_combine_context", started, timings_ms)
        ...
        _log_step_end("8_build_prompt", started, timings_ms)
        ...
        _log_step_end("9_call_llm_model", started, timings_ms)

        started = _log_step_start("10_response")
        yield build_citations_payload(context.citations)
        _log_step_end("10_response", started, timings_ms)
