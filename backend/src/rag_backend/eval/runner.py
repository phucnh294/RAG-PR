...
async def run_golden_set(
    ...
    filename_to_id = {doc.filename: doc.id for doc in await postgres_store.list_documents(user.id)}

    results: list[QueryEvalResult] = []
    for entry in entries:
        try:
            results.append(await _evaluate_entry(entry, filename_to_id, k, user))
        except Exception:
...
async def _evaluate_entry(
    entry: GoldenEntry, filename_to_id: dict[str, str], k: int, user: CurrentUser
) -> QueryEvalResult:
    ...
    chunks = [chunk async for chunk in run_retrieval(entry.query, user)]
    ...
    if entry.category == "real" and expected_document_id is not None:
        matched_rank = await _rank_expected_document(entry.query, expected_document_id, k, user)
...
async def _rank_expected_document(
    query: str, expected_document_id: str, k: int, user: CurrentUser
) -> int | None:
    ...
    scored_chunks = await similarity_search(embedded, state, top_k=k)
