...
def test_run_indexing_marks_document_ready_and_stores_chunks() -> None:
    ...
    chunks = dummy_store.get_chunks(record.id)
    assert len(chunks) > 0
    assert all(len(chunk.embedding) == settings.embedding_dimension for chunk in chunks)


def test_run_indexing_marks_document_failed_for_unsupported_pdf() -> None:
    record = dummy_store.add_document(
    ...
    assert dummy_store.get_chunks(record.id) == []
