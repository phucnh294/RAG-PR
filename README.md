...
> **Status:** early-stage. The upload + chat API and UI are working end-to-end. Chat answers are now generated by a real local LLM (Ollama), but retrieval is still an in-memory dummy store seeded with sample documents — real embeddings/pgvector retrieval is the next feature. See [Plan & documentation](#plan--documentation) for the full roadmap.
...
| Embeddings *(planned)* | Ollama, `nomic-embed-text`, in a second dedicated container |
| Vector store *(planned)* | Postgres + pgvector |
...
Brings up all three containers: `llm-model` (Ollama), `backend` (FastAPI), `frontend` (nginx-served React build).
...
Once all three services show `healthy`:
...
- Retrieval and citations come from an in-memory, keyword-matched dummy store seeded with 3 sample documents — not real embeddings or vector search yet.
- No authentication, rate limiting, or persistent database — the backend's document store resets on restart.
